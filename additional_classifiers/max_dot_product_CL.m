classdef max_dot_product_CL

% max_dot_product_CL is a classifier object (CL) 
%  that learns a mean population vector (template) for each class from the 
%  training data.  When the classifier is tested, the dot product
%  is calculated between a test point and each of the class templates, and the 
%  class with the largest dot prodcut between the test point and the templates is selected as the label
%  (and the decision values are the dot product values).  If there is a only one
...
%  returned as decision values). If two or more classes with the same dot product value
...
%  and the dot product is not divided by the norms of the template or training point.
%
%
...
%==========================================================================    
  ...
  properties 
    ...
    methods 
        ...
        % constructor 
        function cl = max_dot_product_CL
        end
        ...
        function [predicted_labels decision_values] = test(cl, XTe)
            ...
            if size(XTe, 1) > 1   % if each data point is only 1 dimensional (could return an error, but instead just returning the template that had the closest value)
                ...
                template_corrcoeffs = XTe' * cl.templates;
